...
public class RestService {
    ...
    RestService(final KafkaStreams streams) {
    ...
    public KeyValueBean byKey(@PathParam("storeName") final String storeName,
            @PathParam("key") final Long key) {

        // Lookup the KeyValueStore with the provided storeName
        final ReadOnlyKeyValueStore<Long, Long> store = streams.store(storeName, QueryableStoreTypes.<Long, Long>keyValueStore());
        if (store == null) {
     ...
     * @return A List of {@link KeyValueBean}s representing all of the
     * key-values in the provided store that fall withing the given range.
     */
    @GET()
    ...
    public List<KeyValueBean> windowedByKey(@PathParam("storeName") final String storeName,
        ...
        while (results.hasNext()) {
            ...
            windowResults.add(new KeyValueBean(next.key, next.value));
        }
    ...
    private List<KeyValueBean> rangeForKeyValueStore(final String storeName,
            final Function<ReadOnlyKeyValueStore<Long, Long>, KeyValueIterator<Long, Long>> rangeFunction) {

        // Get the KeyValue Store
        final ReadOnlyKeyValueStore<Long, Long> store = streams.store(storeName, QueryableStoreTypes.keyValueStore());
        if (store == null) {
            throw new NotFoundException();
        }
        ...
        final KeyValueIterator<Long, Long> range = rangeFunction.apply(store);

        // Convert the results
        while (range.hasNext()) {
            final KeyValue<Long, Long> next = range.next();
